...
def main(args: argparse.Namespace):
    ...
    sampling_params = SamplingParams(
        n=args.n,  # 每个提示生成的序列数
        temperature=0.0 if args.use_beam_search else 1.0,  # beam search时使用确定性输出
        top_p=1.0,
        use_beam_search=args.use_beam_search,
        ignore_eos=True,  # 忽略结束符号
        max_tokens=256,  # 预热阶段固定生成256个token
    )
    ...
    sampling_params = SamplingParams(
        ...
        use_beam_search=args.use_beam_search,
        ignore_eos=True,
        max_tokens=args.output_len,  # 使用指定的输出长度
    )
    ...
    throughput = total_tokens / latency  # 计算吞吐量（tokens/秒）

    # 初始化GPU功耗监控器
    power_profile = gpuPowerProbe(interval=0.25, gpu_id=0)  # 每0.25秒采样一次

    # 测量功耗
    time.sleep(1)  # 等待系统稳定
    power_profile.start()
    llm.generate(dummy_inputs, sampling_params=sampling_params, use_tqdm=False)
    time.sleep(1)  # 确保采集完整的功耗数据
